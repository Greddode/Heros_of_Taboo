...
static void DrawStatsTab(const GameWorld* game, const InventoryUIState* ui, int ix, int iy, int iw, int ih) {
    ...
    DrawText(buf, col1x + (int)(2 * scale), c1y, textSize, cBlack); c1y += gap; lineIdx++;
    ...
    DrawText(buf, col1x + (int)(2 * scale), c1y, derivedSize, cBlack); c1y += gap; lineIdx++;
    ...
    if (pStatPoints > 0) {
        ...
        DrawText("STR (+1)", col2x + (int)(2 * scale), c2y, allocSize, cBlack); c2y += gap; lineIdx2++;
        DrawText("DEX (+1)", col2x + (int)(2 * scale), c2y, allocSize, cBlack); c2y += gap; lineIdx2++;
        DrawText("MGC (+1)", col2x + (int)(2 * scale), c2y, allocSize, cBlack); c2y += gap; lineIdx2++;
        DrawText("CON (+1)", col2x + (int)(2 * scale), c2y, allocSize, cBlack); c2y += gap; lineIdx2++;
        DrawText("LCK (+1)", col2x + (int)(2 * scale), c2y, allocSize, cBlack); c2y += gap; lineIdx2++;
    } else {
...
void InventoryUI_Render(GameWorld* game, const InventoryUIState* ui) {
    ...
    if (ui->tab == INV_TAB_STATS) {
        ...
        if (game->inventorySlotCount == 0)
            DrawText("Q / E to switch tabs  |  I / ESC to close", ix + (int)(16 * scale), iy + ih - (int)(28 * scale), footerSize, DARKGRAY);
        else if (ui->subState == INV_BROWSE)
            DrawText("ENTER to select action  |  Q / E to switch tabs  |  I / ESC to close", ix + (int)(16 * scale), iy + ih - (int)(28 * scale), footerSize, DARKGRAY);
        else
            DrawText("UP/DOWN to choose  |  ENTER to confirm  |  ESC to go back", ix + (int)(16 * scale), iy + ih - (int)(28 * scale), footerSize, DARKGRAY);
    } else {
        DrawText("Q / E to switch tabs", ix + (int)(16 * scale), iy + ih - (int)(28 * scale), footerSize, DARKGRAY);
    }
}
